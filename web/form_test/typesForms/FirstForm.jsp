...
<body>
    <c:set var="num" scope="request" value="${idTest}" />
        <form action="testdemonstration?type=first&idTest=<c:out value="${num}"/>" method="post">
            <c:forEach var="res" items="${result}">
                <p><b> <c:out value="${res.question}"/></b></p>
                <c:forEach var="ans" items="${res.answers}">
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="answer" id="exampleRadios" value="<c:out value="${ans}"/>">
